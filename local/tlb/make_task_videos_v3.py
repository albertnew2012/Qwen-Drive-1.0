#!/usr/bin/env python
"""v3: adds --hide-gt-boxes, for a video showing only what the system predicts.
...
def render(task, rec, vqa_pred, K, idx, n, tally, fts, hide_gt=False):
    ...
    for task in args.tasks:
        ...
        for seg in order:
            ...
            for i, rec in enumerate(frames):
                K = json.load(open(ROOT / rec['gt_json']))['sensor']['CAM_FRONT']['intrinsic']['K'] \
                    if 'gt_json' in rec else [[1252.8, 0, 826.6], [0, 1252.8, 470.0], [0, 0, 1]]
                ...
                render(task, rec, vqa.get(rec['key']), K, i, len(frames), tally, fts,
                       hide_gt=args.hide_gt_boxes).save(tmp / f'{k:05d}.png'); k += 1
