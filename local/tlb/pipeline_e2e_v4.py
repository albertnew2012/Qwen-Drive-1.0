#!/usr/bin/env python
"""v4: the ego set is required to be internally consistent.

v3 note: reports a SET of ego lights and can stabilise them across frames.

v2 note: the selector may be an ensemble.
...
def main():
    ...
    ap.add_argument('--ego-thr', type=float, default=0.35,
                    help='every light above this is an ego light; the median frame has two')
    ap.add_argument('--consistent', action='store_true',
                    help='restrict the ego set to lights agreeing with the voted answer; '
                         'a lane cannot be governed by a red and a green light at once')
    ap.add_argument('--track', type=float, default=0.0,
                    help='causal EMA on P(governs), associated across frames by IoU. '
                         '0 disables; 0.6 keeps 40%% of the previous frame.')
    ...
    with torch.no_grad():
        for r in rows:
            ...
            gov_raw = torch.sigmoid(lg[0, :L]).cpu().numpy()
            if args.track > 0:
                # associate to the previous frame of the SAME segment and smooth
                if prev_state['seg'] != r['segment']:
                    prev_state.update(seg=r['segment'], boxes=[], gov=[])
                sm = gov_raw.copy()
                for j, b in enumerate(boxes):
                    best, bi = 0.0, -1
                    for k, pb in enumerate(prev_state['boxes']):
                        v = _iou(b, pb)
                        if v > best:
                            best, bi = v, k
                    if bi >= 0 and best >= 0.3:
                        sm[j] = args.track * gov_raw[j] + (1-args.track) * prev_state['gov'][bi]
                prev_state['boxes'] = list(boxes)
                prev_state['gov'] = list(sm)
                lg = torch.from_numpy(
                    np.log(np.clip(sm, 1e-6, 1-1e-6) / (1-np.clip(sm, 1e-6, 1-1e-6)))
                ).float().to(device)[None]
                lg = torch.cat([lg, torch.full((1, MAXL-L), -1e4, device=device)], 1)
            ...
            ts = []
            for i in range(L):                       # every light, indexed by box id
                c = crop_one(im, boxes[i])
                t = torch.from_numpy(np.asarray(c, np.uint8).copy())
                ts.append(((t.permute(2, 0, 1).float() / 255.) - 0.5) / 0.5)
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                P_all = torch.softmax(col(torch.stack(ts).to(device)).float(), 1).cpu().numpy()
            P = P_all[order]                         # the vote still uses the top-k
            ...
            gv = torch.sigmoid(lg[0, :L]).cpu().numpy()
            box_col = [['red', 'green', 'yellow'][int(pp.argmax())] for pp in P_all[:, 1:4]]
            rec['box_colour'] = box_col
            raw = [int(i) for i in np.nonzero(gv >= args.ego_thr)[0]]
            if not raw and L:
                raw = [int(gv.argmax())]
            rec['ego_set_raw'] = raw
            rec['ego_raw_inconsistent'] = int(len({box_col[i] for i in raw}) > 1)
            if args.consistent and rec['pred'] is not None:
                keep = [i for i in raw if box_col[i] == rec['pred']]
                rec['ego_set'] = keep or [int(max(raw, key=lambda i: gv[i]))]
            else:
                rec['ego_set'] = raw
            rec['n_ego'] = len(rec['ego_set'])
            rec['ego_boxes'] = [[round(v, 1) for v in boxes[i]] for i in rec['ego_set']]
            rec['ego_colours'] = [box_col[i] for i in rec['ego_set']]
            if args.dump_boxes:
