...
def tl_draw(ax, tl, K, img_w, img_h, _img_arr=None):
    ...
    for j, b in enumerate(tl["boxes"]):
        ...
        if pick:
            _x = min(q[0] for q in p2) * _kx
            _y = min(q[1] for q in p2) * _ky
            ax.text(_x, _y - 4, f"EGO {rng:.0f} m  h {_h:.1f} m", color=col,
                    fontsize=5.5, zorder=7)
    ax.set_xlim(*_xlim); ax.set_ylim(*_ylim)

    # magnified inset: the only place the cuboid's depth is visible at these ranges
    _ego = [j for j in range(len(tl["boxes"])) if j in set(tl.get("ego_set", []))]
    if _ego:
        j = _ego[0]
        b = tl["boxes"][j]; rng, hh = tl["r3"][j]
        col = RGBF[tl["colour"][j]]
        cxp, cyp = (b[0]+b[2])/2, (b[1]+b[3])/2
        half = max(b[2]-b[0], b[3]-b[1]) * 1.15 + 7
        iax = ax.inset_axes([0.70, 0.55, 0.28, 0.42])
        iax.imshow(_img_arr, interpolation="nearest")
        iax.set_xlim((cxp-half)*_kx, (cxp+half)*_kx)
        iax.set_ylim((cyp+half)*_ky, (cyp-half)*_ky)
        iax.set_xticks([]); iax.set_yticks([])
        for sp in iax.spines.values():
            sp.set_color(col); sp.set_linewidth(1.2)
        Z = max(rng, 1.0)
        X, Y = (cxp-cx)*Z/fx, (cyp-cy)*Z/fy
        W = (b[2]-b[0])*Z/fx; H = (b[3]-b[1])*Z/fy; D = 0.30
        pts = [(X+sx_*W/2, Y+sy_*H/2, Z+sz_*D/2)
               for sz_ in (-1, 1) for sy_ in (-1, 1) for sx_ in (-1, 1)]
        q = [((fx*px/max(pz, .1)+cx)*_kx, (fy*py/max(pz, .1)+cy)*_ky) for (px, py, pz) in pts]
        for a_, b_ in ((4, 5), (5, 7), (7, 6), (6, 4), (0, 4), (1, 5), (2, 6), (3, 7)):
            iax.plot([q[a_][0], q[b_][0]], [q[a_][1], q[b_][1]], color=col, lw=1.0, alpha=.55)
        for a_, b_ in ((0, 1), (1, 3), (3, 2), (2, 0)):
            iax.plot([q[a_][0], q[b_][0]], [q[a_][1], q[b_][1]], color=col, lw=1.8)
        iax.set_title(f"ego light, 3D box @ {rng:.0f} m", fontsize=4.6, color=col, pad=1.2)
...
def render_session_frame(frame, result, traj, gt_traj, reasoning=None,
    ...
    for i, cam in enumerate(frame.cam_order):
        ...
        if tl is not None and cam == "CAM_FRONT" and tl.get("boxes"):
            _panel = camera_with_paths(frame, i, cam, pb, pl, traj, gt_traj, l2i[i])
            tl_draw(_ax, tl, frame.cam_intrinsic[i], 1600.0, 900.0,
                    _img_arr=np.asarray(_panel))
