#!/usr/bin/env python
"""v3: reports a SET of ego lights and can stabilise them across frames.
...
def main():
    ...
    with torch.no_grad():
        for r in rows:
            ...
            rec['ego_set'] = [int(i) for i in np.nonzero(gv >= args.ego_thr)[0]]
            if not rec['ego_set'] and L:
                rec['ego_set'] = [int(gv.argmax())]
