#!/usr/bin/env python
"""Lane type: fine-tune the VLM to read its own lane's arrow.

Originally derived from train_vqa_v2.py.
...
QUESTION = ("Look at the arrow painted on the road in your own lane. Which movements does "
...
def build_sample(model, row, cond='full'):
    ...
    # the answer is the permitted-movement set, in a fixed order so the target is unique
    _ans = ", ".join([m for m in ("left", "straight", "right") if m in row["allow"]])
