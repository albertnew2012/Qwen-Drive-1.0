#!/usr/bin/env python
"""v7: cuboid only -- no dropline, no ground ellipse.

v6 note: the 3D cuboid is drawn in a magnified inset where it is actually resolvable.

v5 note: --box3d also drops each light to the road beneath it.
...
def cuboid_inset(rec, j, K, size=300, zoom=None):
    """Magnified crop around one light with its predicted 3D cuboid drawn over it.

    The cuboid is built in metres from the predicted range, then projected with the same
    intrinsics scaled by the zoom factor, so what is drawn is the real predicted box --
    magnified, not redrawn to look good.
    """
    from PIL import Image as _I
    b = rec['boxes'][j]
    rng, hgt = rec['r3'][j]
    src_im = _I.open(ROOT / rec['image']).convert('RGB')
    cx_px, cy_px = (b[0]+b[2])/2, (b[1]+b[3])/2
    half = max(b[2]-b[0], b[3]-b[1]) * 1.9 + 26
    box = (int(cx_px-half), int(cy_px-half), int(cx_px+half), int(cy_px+half))
    crop = src_im.crop(box).resize((size, size), _I.LANCZOS)
    z = size / (2*half)                                   # px in crop per px in source
    dd = ImageDraw.Draw(crop)
    pts, dims = cuboid_from_prediction(b, rng, K, depth_m=0.30)
    proj = project(pts, K)
    P = [((u - box[0]) * z, (v - box[1]) * z) for (u, v) in proj]
    col = RGB.get(rec['box_colour'][j], CYAN)
    dim = tuple(int(c*0.45) for c in col)
    for a, bb in ((4, 5), (5, 7), (7, 6), (6, 4)):        # far face first
        dd.line([P[a], P[bb]], fill=dim, width=2)
    for a, bb in ((0, 4), (1, 5), (2, 6), (3, 7)):        # connecting edges
        dd.line([P[a], P[bb]], fill=dim, width=2)
    for a, bb in ((0, 1), (1, 3), (3, 2), (2, 0)):        # near face on top
        dd.line([P[a], P[bb]], fill=col, width=3)
    for (px, py) in P[:4]:
        dd.ellipse([px-3, py-3, px+3, py+3], fill=col)
    return crop, dims
...
def render(task, rec, vqa_pred, K, idx, n, tally, fts, hide_gt=False, box3d=False):
    ...
    if task == 'task3' and rec.get('boxes'):
        ...
        ch = rec.get('chosen', -1)
        if box3d and 0 <= ch < len(rec['boxes']):
            ins, dims = cuboid_inset(rec, ch, K, size=300)
            canvas.paste(ins, (W - 312, HDR + 292))
            d.rectangle([W-312, HDR+292, W-12, HDR+592],
                        outline=RGB.get(rec['box_colour'][ch], CYAN), width=2)
            d.text((W - 312, HDR + 596),
                   f"ego light, 3D cuboid  {dims[0]:.2f}w x {dims[1]:.2f}h x {dims[2]:.2f}d m",
                   font=ftiny, fill=(190, 190, 200))
